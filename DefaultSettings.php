...
return array(
	...
	'smwgElasticsearchConfig' => [
		...
		'connection' =>[
			...
			// Uses a faster "quick" ping than the standard ElasticLient:ping in
			// order to verify whether a connection can be established to ensure
			// safe.replication
			'use.quick.ping' => true
		],
		'settings' => [
		...
		'replication' => [
			...
			'no.nodes.available.recovery.job.retries' => 5
		],
		'query' => [

			// If for some reason no connection to the ES cluster could be established,
			// use the SQLStore QueryEngine as fallback.
			'no.connection.fallback' => false,
			...
			// By default, equality searches (e.g. [[Has text:Foo]]) are case
			// senstive but as in case of the SMW_FIELDT_CHAR_NOCASE, the search
			// can be altered to find case insensitive matches as well.
			//
			...
			'text.field.case.insensitive' => false
		]
	],
	##
